...
function getPokemon(html, name, nameJa2en) {
	...
	if (
		...
		const evolink = dom.window.document.querySelector(
			'h4 a[title="育成/進化"]',
		);
		if (evolink === null) {
			throw new Error("進化 not found");
		}
		ancestor =
			evolink.parentNode.parentNode.querySelector("p>strong>a")?.textContent;
		// Charmander: [4, 5, 6]
		// Non-evolving: []
		const evoline = [
			...evolink.parentNode.parentNode.querySelectorAll("p>strong>a"),
	...
	// find id
	const idMatch = dom.window.document
		.querySelector("ins")
	...
	if (carryLimitTable === null) {
		throw new Error("carry limit table not found");
	}
	...
	const skillJa = tds[1].textContent
		...
		.replace("（", "(")
		.replace("）", ")");
	if (!(skillJa in skills)) {
		throw new Error(`Unknown skill: ${skillJa}`);
	}
	...
	if (typeTable === null) {
		throw new Error("type table not found");
	}
	tds = typeTable.querySelectorAll("td");
	const specialtyJa = tds[0].textContent;
	if (!(specialtyJa in specialties)) {
		throw new Error(`Unknown specialty: ${specialtyJa}`);
	}
	...
	if (frequencyMatch === null) {
		throw new Error(`frequency not found: ${tds[1].textContent}`);
	}
	...
	const berryJa = tds[2]
		...
		.getAttribute("title")
		.replace("きのみ/", "");
	if (!(berryJa in berryTypes)) {
		throw new Error(`Unknown berry: ${berryJa}`);
	}
	...
	if (sleepTypeTable === null) {
		throw new Error("sleep type table not found");
	}
	tds = sleepTypeTable.querySelectorAll("td");
	const sleepTypeJa = tds[0]?.textContent;
	if (!(sleepTypeJa in sleepTypes)) {
		throw new Error(`Unknown sleep type: ${sleepTypeJa}`);
	}
	...
	if (fpMatch === null) {
		throw new Error(`fp not found: ${tds[1].textContent}`);
	}
	...
	if (ingTable === null) {
		throw new Error(`ing table not found`);
	}
