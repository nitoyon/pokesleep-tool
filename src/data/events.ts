...
export class BonusEventData {
	...
	isTarget(pokemon: PokemonData): boolean {
		if (
			this.target?.specialty !== undefined &&
 ...
 * @returns Bonus value.
 */
export function getDrowsyBonus(
	date: Date,
	overrideEvents?: DrowsyEventData[],
): number {
 ...
 */
export function getActiveHelpBonus(
	date: Date,
	overrideEvents?: BonusEventData[],
): BonusEventData[] {
 ...
 */
export function getEventBonusIfTarget(
	name: string,
...
export function loadHelpEventBonus(data: unknown): HelpEventBonus {
	...
	if (typeof (data as { target?: unknown }).target === "object") {
		const target = (data as { target?: unknown })
			.target as Partial<TargetPokemon>;
		if (
			typeof target.specialty === "string" &&
			SpecialtyNames.includes(target.specialty)
		) {
			ret.target.specialty = target.specialty as PokemonSpecialty;
		}
		if (typeof target.type === "string" && PokemonTypes.includes(target.type)) {
	...
	if (typeof (data as { effects?: unknown }).effects === "object") {
		...
		if (
			typeof effects.carryLimitMul === "number" &&
