...
const SelectEx = React.memo(
	({
		...
		const onMenuClick = React.useCallback(
			(child: React.ReactElement<SelectExChildProps>) => {
		...
		const childrenArray = React.Children.toArray(
			children,
		) as React.ReactElement<SelectExChildProps>[];
		...
		const menuItems = childrenArray.map((child) => {
			...
			return React.cloneElement(child, {
				onClick: () => {
					onMenuClick(child as React.ReactElement<SelectExChildProps>);
				},
				selected,
			});
		});
		...
		return (
			<>
				...
				<PopperMenu anchorEl={anchorRef.current} open={open} onClose={onClose}>
					<MenuList style={menuSx}>{menuItems}</MenuList>
				</PopperMenu>
			</>
		);
	},
