...
const EnergyChartAxis = React.memo(
	({
		width,
		height,
		period,
		...
		return (
			<>
				...
				<g fontSize="60%" fill="#999" textAnchor="end">
					...
					<text alignmentBaseline="central" x="-5" y={y80}>
						80
					</text>
					<text alignmentBaseline="central" x="-5" y={y60}>
						60
					</text>
					<text alignmentBaseline="central" x="-5" y={y40}>
						40
					</text>
					<text alignmentBaseline="central" x="-5" y={y20}>
						20
					</text>
				...
				<g
					...
					textAnchor="start"
				>
					<text alignmentBaseline="central" x="0" y={e20}>
						2.0
					</text>
					<text alignmentBaseline="central" x="0" y={e15}>
						1.5
					</text>
					<text alignmentBaseline="central" x="0" y={e10}>
						1.0
					</text>
				</g>
				<g
					transform={`translate(0,${ye + 3})`}
					fontSize="60%"
					fill="#999"
					textAnchor="middle"
				>
					<text alignmentBaseline="hanging" x={x9} y="0">
						9:00
					</text>
					<text alignmentBaseline="hanging" x={x12} y="0">
						12:00
					</text>
					<text alignmentBaseline="hanging" x={x15} y="0">
						15:00
					</text>
					<text alignmentBaseline="hanging" x={x18} y="0">
						18:00
					</text>
					<text alignmentBaseline="hanging" x={x21} y="0">
						21:00
					</text>
					<text alignmentBaseline="hanging" x={x24} y="0">
						24:00
					</text>
					<text alignmentBaseline="hanging" x={x27} y="0">
						3:00
					</text>
					<text alignmentBaseline="hanging" x={x6} y="0">
						6:00
					</text>
				</g>
				<rect
					x={(width * sleepTime) / 1440}
					y="0"
					width={(width * (1440 - sleepTime)) / 1440}
					height={height}
					fill="#000080"
					fillOpacity="0.08"
				/>
				{period < 24 && (
					<rect
						x={(width * period * 60) / 1440}
						y="0"
						width={(width * (1440 - period * 60)) / 1440}
						height={height}
						fill="#6666tt"
						fillOpacity="0.15"
					/>
...
const EnergyLine = React.memo(
	({
		...
		return (
			<>
				<polygon
					points={energyFillPoints1.join(" ")}
					stroke="none"
					fill="#1976d2"
					fillOpacity={0.2}
				/>
				<polyline
					points={energyPoints1.join(" ")}
					stroke="#1976d2"
					fill="none"
				/>
				{energyFillPoints2.length > 0 && (
					<>
						<polyline
							points={energyPoints2.join(" ")}
							stroke="#000000"
							fill="none"
						/>
						<polygon
							points={energyFillPoints2.join(" ")}
							stroke="none"
							fill="#000000"
							fillOpacity={0.2}
						/>
...
const EnergyHover = React.memo(
	({
		...
		if (eventIndex <= 0) {
			...
			const rate =
				(minute - curEvent.minutes) / (nextEvent.minutes - curEvent.minutes);
			energy =
		...
		// find efficiency
		let efficiency = result.efficiencies.find(
			(e) => e.start <= minute && minute < e.end,
		);
		...
		return (
			<>
				...
				<circle
					fill={isSnacking ? "#000" : "#1976d2"}
					cx={x}
					cy={energy2y(energy, height)}
					r="3"
				/>
				<circle
					fill="#ff0000"
					cx={x}
					cy={efficiency2y(efficiency.efficiency, height)}
					r="3"
				/>
