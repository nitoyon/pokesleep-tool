...
export class PokemonFilterConfig {
	...
	load(json: unknown) {
		...
		if (typeof (json as { filterType: unknown }).filterType === "string") {
			const filterType = (json as { filterType: unknown })
				.filterType as PokemonType;
			if (PokemonTypes.includes(filterType)) {
				this.filterType = filterType;
			}
		}
		if (Array.isArray((json as { filterSpecialty: unknown }).filterSpecialty)) {
			const filterSpecialty = (json as { filterSpecialty: unknown })
		...
		if (typeof (json as { filterEvolve: unknown }).filterEvolve === "string") {
			const filterEvolve = (json as { filterEvolve: unknown }).filterEvolve as
				| "all"
				| "non"
				| "final";
		...
		if (
			typeof (json as { ingredientName: unknown }).ingredientName === "string"
		) {
			const ingredientName = (json as { ingredientName: unknown })
		...
		if (typeof (json as { ingredientA: unknown }).ingredientA === "boolean") {
			this.ingredientA = (json as { ingredientA: unknown })
				.ingredientA as boolean;
		}
		if (typeof (json as { ingredientB: unknown }).ingredientB === "boolean") {
			this.ingredientB = (json as { ingredientB: unknown })
				.ingredientB as boolean;
		}
		if (typeof (json as { ingredientC: unknown }).ingredientC === "boolean") {
			this.ingredientC = (json as { ingredientC: unknown })
				.ingredientC as boolean;
		}
		if (Array.isArray((json as { mainSkillNames: unknown }).mainSkillNames)) {
			const mainSkillNames = (json as { mainSkillNames: unknown })
				.mainSkillNames as MainSkillName[];
	...
	filter(options: PokemonOption[], text: string): PokemonOption[] {
		...
		if (this.mainSkillNames.length > 0) {
			ret = ret.filter((option) =>
				this.mainSkillNames.some((x) => matchMainSkillName(option, x)),
			);
		}
...
const PokemonSelectDialog = React.memo(
	({
		...
		const [config_, setConfig] = React.useState<PokemonDialogConfig | null>(
			null,
		);

		// initialize config
		const config: PokemonDialogConfig =
			config_ === null ? loadPokemonDialogConfig() : config_;
		...
		const filterOptions = useCallback(
			(options: PokemonOption[], state: FilterOptionsState<PokemonOption>) => {
		...
		const onAutocompleteChange = useCallback(
			(_: React.SyntheticEvent, newValue: PokemonOption | string | null) => {
				if (newValue === null) {
					return;
				}
			...
			(option: PokemonOption) => {
				...
				switch (sortType) {
					...
					case "type":
						return option.type;
					case "arrival":
						return option.arrival === releaseDate
							? "initial pokemon"
		...
		return (
			<StyledDialog open={open} onClose={closeHandler}>
				<Autocomplete
					options={options}
					fullWidth
					open
							...
							<MenuItem key={key} {...others}>
								...
								<footer>
									...
									{option.localName.endsWith(")") && (
										<small>{option.localName.replace(/^[^(]+/, "")}</small>
									)}
								</footer>
							</MenuItem>
						);
