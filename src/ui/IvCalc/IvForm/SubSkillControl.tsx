...
const SubSkillButton = React.memo(
	({
		level,
		value,
		disabled,
...
const EditSubSkillDialog = React.memo(
	({
		open,
		value,
		level,
		...
		const onClick = React.useCallback(
			(subSkillType: SubSkillType) => {
				...
				if (newValue.lv10 === null) {
					onLevelChange(10);
				} else if (newValue.lv25 === null) {
					onLevelChange(25);
		...
		return (
			<Dialog open={open} onClose={onClose}>
				...
				>
					<Trans
						i18nKey="select subskill at level xxx"
						components={{
							level: (
								<strong>
									{t("level")} {level}
...
export const EditSubSkillControl = React.memo(
	({
		subSkill2Badge,
		onClick,
	}: {
		...
		const createButton = React.useCallback(
			(name: SubSkillType, label: string | undefined) => {
				const subSkill = new SubSkill(name);
				const color = subSkill.isGold
					? "gold"
					: subSkill.isBlue
						? "blue"
						: "white";
