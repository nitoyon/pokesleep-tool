		...
		const onSpecialtyClick = useCallback(
			(val: PokemonSpecialty) => {
				onChange(
					new PokemonFilterConfig({
						...value,
						filterSpecialty: value.filterSpecialty.includes(val) ? [] : [val],
					}),
				);
			},
			[value, onChange],
		);
		const onEvolveChange = useCallback(
			(_: React.MouseEvent, val: string | null) => {
				if (val === null || value.filterEvolve === val) {
					return;
				}
				onChange(
					new PokemonFilterConfig({
						...value,
						filterEvolve: val as "all" | "non" | "final",
					}),
				);
			},
			[value, onChange],
...
const IngredientTab = React.memo(
	({
		value,
		onChange,
	}: {
		...
		const onIngClick = useCallback(
			(selected: IngredientName) => {
				const ingredientName =
					value.ingredientName === selected ? "unknown" : selected;
				onChange(new PokemonFilterConfig({ ...value, ingredientName }));
			},
			[value, onChange],
		);

		const onIngAClick = useCallback(
			(e: React.ChangeEvent<HTMLInputElement>) => {
				onChange(
					new PokemonFilterConfig({ ...value, ingredientA: e.target.checked }),
				);
			},
			[value, onChange],
		);
		const onIngBClick = useCallback(
			(e: React.ChangeEvent<HTMLInputElement>) => {
				onChange(
					new PokemonFilterConfig({ ...value, ingredientB: e.target.checked }),
				);
			},
			[value, onChange],
		);
		const onIngCClick = useCallback(
			(e: React.ChangeEvent<HTMLInputElement>) => {
				onChange(
					new PokemonFilterConfig({ ...value, ingredientC: e.target.checked }),
				);
			},
			[value, onChange],
		);
