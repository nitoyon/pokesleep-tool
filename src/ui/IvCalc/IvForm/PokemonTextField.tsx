			...
			<div>
				{fixMode ? (
					...
					<TextLikeButton
						...
						style={{ width: "10.2rem", fontSize: "0.9rem" }}
						className={open ? "focused" : ""}
					>
						{selectedOption.localName.replace(/\(.+/, "")}
						{selectedOption.localName.endsWith(")") && (
							<small>{selectedOption.localName.replace(/^[^(]+/, "")}</small>
						)}
					</TextLikeButton>
				)}
 ...
 */
const EvolveButton = React.memo(
	({ iv, onChange }: { iv: PokemonIv; onChange: (name: string) => void }) => {
		const { t } = useTranslation();
		const [evolveButtonEl, setEvolveButtonEl] =
			React.useState<null | HTMLElement>(null);
		const hasEvolutionaryLine = Boolean(iv.pokemon.ancestor !== null);

		// menu item click handler
		const onEvolveMenuItemClick = React.useCallback(
			(e: React.MouseEvent) => {
		...
		for (const p of pokemonsInEvoLine) {
			evolveMenuItems.push(
				<MenuItem key={p.name} value={p.name} onClick={onEvolveMenuItemClick}>
					<ListItemIcon>
						{p.name === iv.pokemon.name ? <CheckIcon /> : <Icon />}
					</ListItemIcon>
					{t(`pokemons.${p.name}`)}
				</MenuItem>,
			);
...
const CandyButton = React.memo(({ onClick }: { onClick: () => void }) => {
	return (
		<StyledCandyButton size="small" onClick={onClick}>
			<CandyIcon />
		</StyledCandyButton>
	);
});
