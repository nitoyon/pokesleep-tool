	...
	({
		...
		switch (mainSkill) {
			...
			case "Charge Energy S (Moonlight)":
				return (
					<FavoriteBorderIcon sx={{ color: "#ff88aa" }} fontSize="small" />
				);
			case "Energy for Everyone S":
				return (
					<VolunteerActivismIcon sx={{ color: "#ff88aa" }} fontSize="small" />
				);
			...
			case "Helper Boost":
				return (
					<SavedSearchOutlined sx={{ color: "#66cc66" }} fontSize="small" />
				);
			...
			case "Cooking Assist S (Bulk Up)":
				if (second !== true) {
					...
					return (
						<PriorityHighIcon sx={{ color: "#ff0000" }} fontSize="small" />
					);
				}
