...
export const FrequencyInfoPreview = React.memo(
	({
		iv,
		state,
		onStateChange,
		...
		if (state.displayValue === "full") {
			return (
				<FullPreview iv={iv} state={state} onStateChange={onStateChange} />
			);
		} else {
...
const FullPreview = React.memo(
	({
		...
		const convertX = React.useCallback(
			(index: number) => {
		...
		const color = React.useCallback(
			(index: number) =>
				index < chartData.low || chartData.high < index ? "#999" : "#1976d2",
			[chartData],
		);
		...
		const onDistributionModeChange = React.useCallback(
		...
		return (
			<StyledFullPreview ref={chartRef}>
				...
				<BarChart
					width={chartWidth}
					data={
						state.distributionMode === "pmf" ? chartData.pmf : chartData.cdf
					}
					average={chartData.average}
					color={color}
					convertX={convertX}
					formatX={(x) => {
						return formatHoursShort(x, t);
					}}
					formatXDetail={(x) => {
						return formatHoursLong(x, t);
					}}
					...
					formatHover={(index) => {
						...
						return (
							<StyledHover>
								...
								<span className="lbl">{t("help count")}:</span>
								<div>{index}</div>
...
export const FrequencyForm = React.memo(
	({
		iv,
		simple,
		state,
		...
		const onBerryBonusChange = React.useCallback(
			(_: React.MouseEvent, value: string | null) => {
				if (value !== null) {
		...
		const onIngBonusChange = React.useCallback(
			(_: React.MouseEvent, value: string | null) => {
				if (value !== null) {
		...
		const onCarryLimitBonusChange = React.useCallback(
			(_: React.MouseEvent, value: string | null) => {
				if (value === null) {
					return;
				}
		...
		const onExpertModeChange = React.useCallback(
			(e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const onExpertIngBonusChange = React.useCallback(
			(_: React.MouseEvent, value: string | null) => {
				if (value !== null) {
		...
		const onValueChange = React.useCallback(
			(_: React.MouseEvent, value: string | null) => {
				...
				onStateChange({
					...state,
					displayValue: value as "frequency" | "count" | "full",
				});
			},
			[state, onStateChange],
		);
		...
		const carryLimit =
			state.carryLimitMul > 1 ? state.carryLimitMul : state.carryLimitAdd;
		return (
			<StyledFrequencyControls>
				...
				<Collapse in={state.displayValue === "full"}>
					...
					{!simple && (
						<>
							<div className="line">
								<span className="lbl">{t("event bonus")}:</span>
							</div>
							<div className="line">
							...
							<div className="line">
								<span className="indent lbl">{t("ingredient")}:</span>
								<ToggleButtonGroup
									size="small"
									exclusive
									value={state.ingredient}
									onChange={onIngBonusChange}
								>
									<ToggleButton value={0}>{t("none")}</ToggleButton>
									<ToggleButton value={1}>+1</ToggleButton>
								</ToggleButtonGroup>
							...
							<div className="line">
								<span className="indent lbl">{t("carry limit")}:</span>
								<ToggleButtonGroup
									size="small"
									exclusive
									value={carryLimit}
									onChange={onCarryLimitBonusChange}
								>
									<ToggleButton value={0}>{t("none")}</ToggleButton>
				...
				{!simple && (
					<>
						<div className="line">
							<span className="lbl">{t("expert mode")}:</span>
							<Switch
						...
						<Collapse in={state.expertMode}>
							<div className="line">
								...
								<ToggleButtonGroup
									size="small"
									exclusive
									value={state.expertBerry}
									onChange={onExpertBerryChange}
								>
						...
						>
							<div className="line">
								<span className="indent lbl">{t("expert effect")}:</span>
								<ToggleButtonGroup
									size="small"
									exclusive
									value={state.expertIngBonus}
									onChange={onExpertIngBonusChange}
								>
									<ToggleButton value={1}>
										{t("expert ing effect")}
									</ToggleButton>
				...
				<div className="line">
					<span className="lbl">{t("value")}:</span>
				</div>
				<div className="value">
					<ToggleButtonGroup
						exclusive
