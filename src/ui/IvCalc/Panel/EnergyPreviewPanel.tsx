	...
	({
		...
		const convertToVal = (rate: number) => {
			switch (display) {
				...
				case "count":
					return t("help count per hour", {
						n: (3600 / baseFreq / rate).toFixed(2),
					});
				case "pity":
		...
		return (
			<StyledEnergyPreview>
				<span className="energy">
					...
					81{t("range separator")}150
				</span>
				<span>{val100}</span>
				<span className="energy">
					<EnergyIcon energy={80} />
					61{t("range separator")}80
				</span>
				<span>{val80}</span>
				<span className="energy">
					<EnergyIcon energy={60} />
					41{t("range separator")}60
				</span>
				<span>{val60}</span>
				<span className="energy">
					<EnergyIcon energy={40} />1{t("range separator")}40
				</span>
				<span>{val40}</span>
				<span className="energy">
					<EnergyIcon energy={20} />0
				</span>
				<span>{val0}</span>
			</StyledEnergyPreview>
		);
