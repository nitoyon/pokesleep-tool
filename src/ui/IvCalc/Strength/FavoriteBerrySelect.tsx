...
const FavoriteBerrySelect = React.memo(
	({
		...
		const onFavoriteBerryChange1 = React.useCallback(
			(type: PokemonType) => {
				const favoriteType = [
					value.favoriteType[0] ?? "normal",
		...
		const onFavoriteBerryChange2 = React.useCallback(
			(type: PokemonType) => {
				const favoriteType = [
					value.favoriteType[0] ?? "normal",
					value.favoriteType[1] ?? "normal",
		...
		const onFavoriteBerryChange3 = React.useCallback(
			(type: PokemonType) => {
				const favoriteType = [
					value.favoriteType[0] ?? "normal",
					value.favoriteType[1] ?? "normal",
		...
		const onMainBerryChange = React.useCallback(
			(type: PokemonType) => {
				...
				<section>
					...
					<TypeSelectContainer>
						<span>
							...
							<footer className={reasons[0].replace(/ /g, "-")}>
								{t(reasons[0])}
							</footer>
						</span>
						<span>
							{expert && <header>{t("sub1")}</header>}
							{reasons[1] === "random" ? (
								<TypeSelect
									type={types[1]}
									size="small"
							...
							<footer className={reasons[1].replace(/ /g, "-")}>
								{t(reasons[1])}
							</footer>
						</span>
						<span>
							{expert && <header>{t("sub2")}</header>}
							{reasons[2] === "random" ? (
								<TypeSelect
									type={types[2]}
									size="small"
							...
							<footer className={reasons[2].replace(/ /g, "-")}>
								{t(reasons[2])}
							</footer>
						</span>
					</TypeSelectContainer>
...
const ExpertEffectSelect = React.memo(
	({
		value,
		onChange,
	}: {
		...
		return (
			<Select variant="standard" value={value} onChange={onChangeHandler}>
				<MenuItem value="berry">{t("expert berry effect")}</MenuItem>
				<MenuItem value="ing">
					{t("expert ing effect")}
					<br />
					<small>{t("expert ing effect2")}</small>
				</MenuItem>
