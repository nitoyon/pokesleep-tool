			...
			<StyledBerryIngSkillStrengthView>
				<h2>
					...
					<span className="strength">
						{formatWithComma(Math.round(result.totalStrength))}
					</span>
					...
					{decendants.length === 1 && (
						<span className="evolved">
							{t("strength of x", {
								x: t(`pokemons.${pokemonIv.pokemon.name}`),
							})}
						</span>
					)}
					{decendants.length > 1 && (
						<span className="evolved">
							<Select
								variant="standard"
								value={pokemonIv.pokemon.id.toString()}
				...
				<section>
					...
					<footer>
						...
						<div>
							{round1(result.ingHelpCount)}
							{t("times unit")}
						</div>
					</footer>
				</section>
				<section>
					<h3 style={{ background: "#44a2fd" }}>
						{t("skill")}
				...
				{settings.period > whistlePeriod && (
					<footer>
						{result.energy.showSkillStock && settings.period >= 24 ? (
							<>
								<span>
									{t("full inventory while sleeping (short)")}:{" "}
				...
				{settings.period < 0 && (
					<footer>
						<span>
							{t("help yield")}: {round1(getHelpYield(settings, strength))}
							{t("berry unit")}/{Math.abs(settings.period)}
							{t("help unit")}
						</span>
						<span>
							{t("required helper boost")}:{" "}
							{round1(getRequiredHelperBoost(settings, strength, result))}
						</span>
				...
				<EnergyDialog
					...
					onClose={onEfficiencyDialogClose}
					iv={pokemonIv}
					result={result}
					parameter={settings}
					dispatch={dispatch}
				/>
				<HelpStackDialog
					open={helpStockDialogOpen}
					onClose={onHelpStockDialogClose}
					parameter={settings}
					strength={strength}
					result={result}
					dispatch={dispatch}
