...
const HelpStackDialog = React.memo(
	({
		...
		return (
			<StyledHelpStackDialog open={open} onClose={onClose}>
				<article>
					<section className="first">
						<h2>{t("help yield")}:</h2>
						<span>
							{round1(getHelpYield(parameter, strength))}
							{t("berry unit")}/{Math.abs(parameter.period)}
							{t("help unit")}
						</span>
					</section>
					<footer>
						{t("help yield description", { n: Math.abs(parameter.period) })}
					</footer>
					<section>
						<h2>{t("helps for cap")}:</h2>
						<span>
							{round1(getHelpsForCap(strength, result))}
							{t("help unit")}
						</span>
					</section>
					...
					<section>
						...
						<span>
							{round1(getRequiredHelperBoost(parameter, strength, result))}
							{t("times unit")}
						</span>
					</section>
				...
				<footer>
					...
					<section>
						...
						<span>
							{getSkillValue(
								"Helper Boost",
								parameter.helperBoostLevel,
								parameter.helperBoostSpecies,
							)}
							{t("times unit")}
						</span>
					</section>
