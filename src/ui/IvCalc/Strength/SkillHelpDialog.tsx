...
const SkillHelpDialog = React.memo(
	({
		...
		const versatileSkillLevel = Math.min(
			skillLevel,
			getMaxSkillLevel(skillName),
		);
		const bonus = strength.bonusEffects;
		const [skillValueText, skillValueFooter] = getSkillValueText(
			strength,
			versatileSkillLevel,
			t,
		);
		const [skillValueText2, skillValueFooter2] = getSkillValueText2(
			strength,
			skillLevel,
			t,
		);
				...
				<DialogContent>
					<article>
						{!isCountOnly && (
							<>
								...
								{skillValueFooter !== null && (
									<footer>{skillValueFooter}</footer>
								)}
							</>
						)}
						<div>
					...
					{result.skillValue2 !== 0 && (
						<>
							<header className="second-skill" style={{ marginTop: "1.2rem" }}>
								<article>
									{skillName === "Ingredient Magnet S (Plus)" ? (
										...
										<MainSkillIcon
											second
											mainSkill={
												iv.pokemon.skill /* show candy icon for versatile */
											}
										/>
									)}
								...
								<footer>
									<span className="box box4">
										{formatWithComma(result.skillValuePerTrigger2)}
									</span>
									<> × </>
									<span className="box box2">{round2(result.skillCount)}</span>
								</footer>
							...
							<article>
								...
								{skillValueFooter2 !== null && (
									<footer>{skillValueFooter2}</footer>
								)}
							</article>
						</>
					)}
					...
					>
						<div className="skillLevelNotice">
							<Trans
								i18nKey={
									settings.maxSkillLevel
										? "max skill level affected"
										: "skill level bonus affected"
								}
								components={{
									level: <strong>{skillLevel}</strong>,
									bonus: (
										<strong>
											{bonus.skillLevelReason === "event+ex"
												? t("event bonus") +
													t("text separator") +
													t("expert mode")
												: bonus.skillLevelReason === "event"
													? t("event bonus")
													: t("expert mode")}
										</strong>
									),
...
function getSkillValueText(
	...
	if (skill === "Energizing Cheer S") {
		...
		return getEnergyRecoveryValueText(
			val,
			skillLevel,
			t,
	...
	if (skill === "Energizing Cheer S (Heal Pulse)") {
		const val = getSkillValue(skill, skillLevel);
		return getEnergyRecoveryValueText(val, skillLevel, t, t("e4e per pokemon"));
	}
...
function getSkillValueText2(
	...
	if (skill === "Ingredient Draw S (Super Luck)") {
		return getSuperLuckShardText(
			strength,
			skillLevel,
			t,
			t("expected dream shard"),
		);
	}
...
function getChargeEnergyValueText(
	strength: PokemonStrength,
	skillLevel: number,
	t: typeof i18next.t,
): [React.ReactNode, React.ReactNode] {
	...
	const text = t("value per skill", {
		value: t("nature effect.Energy recovery"),
	});
	const val = getSkillValue(skill, skillLevel);
	return [
		<>
...
function getChargeStrengthValueText(
	strength: PokemonStrength,
	skillLevel: number,
	t: typeof i18next.t,
): [React.ReactNode, React.ReactNode] {
...
function getIngredientGetValueText(
	strength: PokemonStrength,
	skillLevel: number,
	t: typeof i18next.t,
): [React.ReactNode, React.ReactNode] {
...
function getDreamShardMagnetValueText(
	strength: PokemonStrength,
	skillLevel: number,
	t: typeof i18next.t,
): [React.ReactNode, React.ReactNode] {
...
function getSuperLuckShardText(
	strength: PokemonStrength,
	skillLevel: number,
	t: typeof i18next.t,
	valueText: string,
	...
	const param = strength.parameter;
	const bonus = getEventBonus(param.event, param.customEventBonus);
	const ingBonus = Math.max(bonus.ingredientMagnet, bonus.dreamShard);
...
function getPlusValueText(
	...
	const param = strength.parameter;
	const bonus = getEventBonus(param.event, param.customEventBonus);
	const ingBonus = Math.max(bonus.ingredientMagnet, bonus.skillIngredient);

	if (ingBonus === 1) {
		return [text, null];
	}

	const val = getSkillSubValue(
		strength.pokemonIv.pokemon.skill,
		skillLevel,
		strength.pokemonIv.pokemon.ing1.name,
	);
	return [
		text,
		<>
...
function getVersatileCandyValueText(
	t: typeof i18next.t,
): [React.ReactNode, React.ReactNode] {
	const text = t("value per skill", { value: t("candy") });
	return [text, null];
...
function getBerryBurstValueText(
	strength: PokemonStrength,
	skillLevel: number,
	t: typeof i18next.t,
	valueText: string,
...
function getNormalSkillValueText(
	t: typeof i18next.t,
	valueText: string,
): [React.ReactNode, React.ReactNode] {
		...
		<>
			...
			<section style={{ paddingLeft: "1rem" }}>
				...
				<span style={{ color: "#999" }}>
					...
					<InfoButton
						onClick={() => onBerryInfoClick(iv.pokemon.type, iv.level)}
					/>
				</span>
			</section>
			{[0, 1, 2, 3].map((i) => (
