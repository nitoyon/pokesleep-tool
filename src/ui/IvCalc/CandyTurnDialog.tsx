...
const CandyTurnDialog = React.memo(
	({
		count,
		iv,
		open,
		...
		const onChange = React.useCallback(
			(name: string, val: number) => {
				...
				switch (name) {
					case "required":
						newConfig.required = val;
						break;
					case "pokemon":
						newConfig.pokemon = val;
						break;
					case "handyS":
						newConfig.handyS = val;
						break;
					case "handyM":
						newConfig.handyM = val;
						break;
					case "handyL":
						newConfig.handyL = val;
						break;
					case "typeS":
						newConfig.typeS = val;
						break;
		...
		return (
			<StyledDialog open={open} onClose={onClose}>
				<StyledContent>
					<header>
						<span className="lbl">{t("required candy")}:</span>
						<section>
							<NumericSliderInput
								min={0}
								max={4000}
					...
					<article className="handy">
						<span className="lbl">{t("handy candy")}:</span>
						<NumericSliderInput
							min={0}
							max={999}
							value={config.handyS}
					...
					<article className="type">
						<span className="lbl">{t("type candy")}:</span>
						<NumericSliderInput
							min={0}
							max={999}
							value={config.typeS}
					...
					<CollapseEx show={add > 0 && usage.length > 0}>
						<table>
							<thead>
								<tr>
									<th colSpan={3} className="handy">
										{t("handy candy")}
									</th>
									<th colSpan={2} className="type">
										{t("type candy")}
									</th>
									<th rowSpan={2} className="sum">
										{t("total")}
									</th>
								</tr>
								<tr>
								...
								{usage.map((x) => {
									...
									return (
										...
										>
											...
											<td className={`handy${x.handyM === 0 ? " empty" : ""}`}>
												{x.handyM}
											</td>
											<td className={`handy${x.handyL === 0 ? " empty" : ""}`}>
												{x.handyL}
											</td>
											<td className={`type${x.typeS === 0 ? " empty" : ""}`}>
												{x.typeS}
											</td>
											<td className={`type${x.typeM === 0 ? " empty" : ""}`}>
												{x.typeM}
											</td>
											<td className="sum">
												{formatWithComma(total)}
												<small> (+{total - add})</small>
											</td>
										</tr>
									);
								})}
