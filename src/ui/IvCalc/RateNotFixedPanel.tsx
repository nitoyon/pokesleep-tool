		...
		return (
			...
			>
				...
				<Button
					...
					sx={{ padding: "0 5px", minWidth: "2rem" }}
				>
					[{t("settings")}]
				</Button>
				<RateNotFixedDialog
...
const RateNotFixedDialog = React.memo(
	({
		dispatch,
		state,
		open,
		...
		const { t } = useTranslation();
		const onMewParamChange = React.useCallback(
			(value: MewParameter) => {
				dispatch({
		...
		const onIngChange = React.useCallback(
			(value: number) => {
				onMewParamChange({
					...state.parameter.mew,
					ing: value,
		...
		const onSkill1Change = React.useCallback(
			(value: number) => {
				onMewParamChange({
					...state.parameter.mew,
					skill1: value,
		...
		const onSkill2Change = React.useCallback(
			(value: number) => {
				onMewParamChange({
					...state.parameter.mew,
					skill2: value,
		...
		const onSkill3Change = React.useCallback(
			(value: number) => {
				onMewParamChange({
					...state.parameter.mew,
					skill3: value,
		...
		const onSuccessChange = React.useCallback(
			(value: number) => {
				onMewParamChange({
					...state.parameter.mew,
					success: value,
		...
		return (
			<Dialog open={open} onClose={onClose}>
				<StyledDialogContent>
					...
					<footer>{t("additional candy description")}</footer>
				</StyledDialogContent>
				<DialogActions>
