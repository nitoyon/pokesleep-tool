...
const PokemonFilterFooter = React.memo(
	({
		...
		const onSortMenuItemSelected = useCallback(
		...
		// Sort menu
		const menuItems = sortTypes.map((type) => (
			<MenuItem
				key={type}
				onClick={onSortMenuItemSelected}
				data-value={type}
				selected={value.sort === type}
			>
				{t(type)}
			</MenuItem>
		));
		...
		return (
			<StyledPokemonFilterFooter>
				<RoundedButton
					...
					key={
						value.isFiltered.toString() /* quick fix for curious label not updated bug on iOS */
					}
					onClick={onFilterButtonClick}
				>
					<SearchIcon style={{ paddingRight: "0" }} />
					{t(value.isFiltered ? "on" : "off")}
				</RoundedButton>
				<RoundedButton
					style={{
						padding: ".2rem .8rem",
				...
				>
					<SortIcon style={{ paddingRight: ".4rem" }} />
					{t(value.sort)}
				</RoundedButton>
				<PopperMenu
					open={sortMenuOpen}
				...
				<SortOrderButton
					className={value.descending ? "desc" : "asc"}
