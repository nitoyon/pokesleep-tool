...
const BoxSortConfigFooter = React.memo(
	({
		...
		const onSkillChange = React.useCallback(
			(value: string) => {
				onChange({
					...sortConfig,
					mainSkill: value as MainSkillName,
				});
			},
			[onChange, sortConfig],
		);
		...
		if (
			sortConfig.sort !== "total strength" &&
					...
					{sortConfig.sort === "skill" && (
						<span className="switch">
							<FormControlLabel
								control={
									<Switch
										checked={parameter.maxSkillLevel}
										size="small"
