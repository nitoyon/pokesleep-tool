...
const TrackingPanel = React.memo(
	({
		...
		const [isScoreTableDialogOpen, setIsScoreTableDialogOpen] =
		...
		const [currentRemainingSeconds, setCurrentRemainingSeconds] =
			React.useState(0);
		...
		const update = React.useCallback(() => {
			...
			setTotalRemainingSeconds(
				_curStage.type === "finish"
					? 0
					: Math.ceil(
							(stages.current[stages.current.length - 2].end.getTime() - now) /
								1000,
						),
			);
		...
		const onStartTracking = React.useCallback(
		...
		const end =
			stages.current.length === 0
				? ""
				: formatTime(
						stages.current[stages.current.length - 2].end,
						i18n.language,
					);
		const completed =
			data.tracking !== undefined && currentStage.type === "finish";
			...
			<StyledTrackingPanel>
				<CollapseEx show={data.tracking !== undefined}>
					<section>
						<article className={currentStage.type}>
							...
							<header>
								<h2>{t(completed ? "tracked" : "tracking")}</h2>
								<h3>
									<strong>{start}</strong>
									{t("range separator")}
									<strong>{end}</strong>
									{!completed && (
										<span className="remain">
											(
											{t("remaining time", {
												time: formatSeconds(totalRemainingSeconds, t),
											})}
											)
										</span>
									)}
						...
						<footer>
							<Trans
								i18nKey={`sleep state.${currentStage.type}`}
								components={{
									time: (
										<strong>{formatSeconds(currentRemainingSeconds, t)}</strong>
									),
				...
				<div className="buttons">
					<Button
						...
						onClick={onScoreTableButtonClick}
					>
						{t("sleep score table")}
					</Button>
					{completed && (
