...
export default class TrackingDetail {
	...
	_calculateStageImpl(): TrackingStage[] {
		...
		let startMinute = 0,
			endMinute = minutesTakenToFallAsleep;
		...
		if (this.score <= 16) {
			// Add 'normal'
			endMinute =
				getAverageMinuteForScore(this.score) + minutesTakenToFallAsleep;
			end = this.getDateFromMinute(endMinute);
			ret.push({
				type: "normal",
				start,
			...
			// Add 'aborting'
			endMinute = Math.min(90, endMinute + minutesRequiredToAbortTracking);
			end = this.getDateFromMinute(endMinute);
			ret.push({
				type: "aborting",
				start,
