...
export function getMinTimeForScore(score: number): AmountOfSleep {
	return new AmountOfSleep(
		Math.max(0, Math.ceil(((score - 0.5) / 100) * 8.5 * 60)),
	);
}
...
export function getMaxTimeForScore(score: number): AmountOfSleep {
	return new AmountOfSleep(
		Math.min(510, Math.ceil(((score + 1 - 0.5) / 100) * 8.5 * 60) - 1),
	);
}
...
export function getScoreRange(
	...
	const secondRange: ScoreRange = getScoreRangeForCount(
		_secondCount,
		data,
		100 - range.minScore,
	);
	...
	ret.firstSleep.maxScore = Math.min(
		range.maxScore,
		100 - secondRange.minScore,
	);
	ret.firstSleep.maxTime = getMaxTimeForScore(ret.firstSleep.maxScore);
	ret.firstSleep.maxPower =
		ret.firstSleep.maxScore * data.strength * data.bonus;

	// calc next strength
	let nextStrength = null;
	if (secondRange.count < 8) {
		const nextCount = (secondRange.count + 1) as PokemonCount;
		const oneMoreRange: ScoreRange = getScoreRangeForCount(
			nextCount,
			data,
			100 - range.minScore,
		);
 ...
 * @returns Score range
 */
export function getScoreRangeForCount(
	count: PokemonCount,
	data: InputAreaData,
