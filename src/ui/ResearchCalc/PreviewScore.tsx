...
export default function PreviewScore(props: PreviewScoreProps) {
	...
	if (!range.canGet || range.tooMuch) {
		return (
			<StyledPreviewScore>
				<div className="preview_warning">{firstElement}</div>
			</StyledPreviewScore>
		);
	}
	if (ranges.secondSleep == null) {
		return (
			<StyledPreviewScore>
				<div className="preview_grid">{firstElement}</div>
			</StyledPreviewScore>
		);
	}
	...
	if (ranges.nextStrength !== null) {
		...
		nextElement = (
			<div className="meet1more">
				<InfoOutlinedIcon />
				<div>
					<Trans
						i18nKey="next strength to get 1 more pokemon"
						components={{
							score1: <strong>{ranges.firstSleep.minScore}</strong>,
							count2: <strong>{count}</strong>,
							strength2: (
								<strong>{t("num", { n: secondRequiredStrength })}</strong>
							),
						}}
					/>
	...
	if (!range.canGet || range.tooMuch) {
		...
		if (range.canGet) {
			...
			const percent = Math.floor(
				((range.requiredStrength - rank.thisStrength) /
					(rank.nextStrength - rank.thisStrength)) *
					100 +
					0.5,
			);
			warningElement = (
		...
		return (
			<>
				{countElement}
				<div className="warning">{warningElement}</div>
			</>
		);
	}
	return (
