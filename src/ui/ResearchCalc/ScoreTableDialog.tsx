...
const ScoreTableDialog = React.memo(
	({ open, onClose, bonus, strength }: ScoreTableDialogProps) => {
		...
		const onMoveClick = React.useCallback(
			(_: React.MouseEvent, val: number | null) => {
				...
				switch (val) {
					case 0:
						element = ref0?.current;
						break;
					case 25:
						element = ref25?.current;
						break;
					case 50:
						element = ref50?.current;
						break;
					case 75:
						element = ref75?.current;
						break;
					case 100:
						element = ref100?.current;
						break;
				}
				if (element !== null) {
		...
		return (
			<StyledDialog open={open} onClose={onClose} scroll="paper">
				...
				<div className="sleep_score_table">{scores}</div>
				<div className="form">
