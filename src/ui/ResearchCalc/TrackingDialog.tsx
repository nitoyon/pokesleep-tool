...
const TrackingDialog = React.memo(
	({
		...
		const [initialStart, initialEnd] = getTrackingPeriod(
			new Date(),
			score,
			i18n.language,
		);
		...
		const updateTime = React.useCallback(
			(s: number) => {
				const [newStart, newEnd] = getTrackingPeriod(
					new Date(),
					s,
					i18n.language,
				);
		...
		const hhmm = new AmountOfSleep(
			Math.max(90, getAverageMinuteForScore(score) + minutesTakenToFallAsleep),
		);
		...
		return (
			<>
						...
						<header>
							...
							<div className="slider">
								...
								<SliderEx
									...
									value={score}
									onChange2={onScoreChange}
								/>
								<ArrowButton
									label="▶"
									disabled={score === 100}
									onClick={onScoreUpClick}
								/>
							</div>
				...
				<MessageDialog
					open={goPlusWarningShown}
					onClose={onGoPlusWarningClose}
					message={
						<>
							<p style={{ fontSize: "0.9rem", margin: 0 }}>
								{t("tracking go plus+ detail.0")}
							</p>
							<p style={{ fontSize: "0.9rem", margin: "0.5rem 0" }}>
								{t("tracking go plus+ detail.1")}
							</p>
							<p style={{ fontSize: "0.9rem", margin: 0 }}>
								{t("tracking go plus+ detail.2")}
							</p>
						</>
					}
				/>
