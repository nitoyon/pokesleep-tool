...
function InputArea({ data, onChange: onchange }: InputAreaProps) {
	...
	const onRankDownClick = useCallback(() => {
		moveRank(-1);
	}, [moveRank]);
	const onRankUpClick = useCallback(() => {
		moveRank(1);
	}, [moveRank]);
	...
	const onBonusChange = React.useCallback(
		(bonus: number) => {
			onchange?.({ bonus });
		},
		[onchange],
			...
			<StyledForm>
				...
				<div>
					...
					<div className="strength_second_line">
						<ArrowButton
							disabled={rank.index === 0}
							label="◀"
							onClick={onRankDownClick}
						/>
						<SliderEx
							className="strength_progress"
							size="small"
							onChange2={onSliderChange}
							min={rank.thisStrength}
							max={rank.nextStrength}
							value={strength}
						/>
						<ArrowButton
							disabled={rank.rankNumber === 20}
							label="▶"
...
const RankTextField = React.memo(
	({ value, onChange, field }: RankTextFieldProps) => {
		const { t } = useTranslation();
...
const StrengthAdornment = (
	<InputAdornment position="start" sx={{ color: "#ff944b" }}>
		<LocalFireDepartmentIcon />
	</InputAdornment>
);

const StrengthTextField = React.memo(
	({ max, value, onChange }: StrengthTextFieldProps) => (
		<StyledStrengthInput
			size="small"
			max={max}
			value={value}
...
const EventBonusTextField = React.memo(
	({ value, onChange }: EventBonusProps) => {
		const { t } = useTranslation();
		...
		return (
			<>
				...
				>
					...
					<MenuItem key="2" value={2} dense>
						×2
					</MenuItem>
					<MenuItem key="2.5" value={2.5} dense>
						×2.5
					</MenuItem>
					<MenuItem key="3" value={3} dense>
						×3
					</MenuItem>
					<MenuItem key="4" value={4} dense>
						×4
					</MenuItem>
				</TextField>
...
const SecondSleepCheckbox = React.memo(
	({ value, onChange }: SecondSleepCheckboxProps) => {
		const { t } = useTranslation();
		return (
			<FormControlLabel
