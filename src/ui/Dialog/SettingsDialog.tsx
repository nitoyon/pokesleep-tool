...
export default function SettingsDialog({
	...
	const buttons = Object.entries(LANGUAGE_NAMES).map(([lng, label]) => (
		<ToggleButton key={lng} value={lng} aria-label={lng}>
			{label}
		</ToggleButton>
	));
	...
	const onCustomIconChange = React.useCallback(
					...
					{app === "IvCalc" && (
						<>
							...
							<TextField
								...
								error={iconUrlInvalid}
								helperText={iconUrlErrorMessage}
							...
							>
								{t("custom icon details")}
								<br />
								{t("custom icon details japanese only")}
							</p>
						</>
					)}
				</DialogContent>
