...
export function calculateHelpCount(
	...
	const awakeSeconds = Math.min(
		1440 * 60 - sleepScoreSeconds,
		param.period * 60 * 60,
	);
	...
	const simulation = new HelpCountSimulation(
		...
		ret.overallSkillRate,
		ret.inventoryBonus,
	);
	calculateAwakeHelpCount(
		ret,
		param,
		awakeSeconds,
		ret.baseFreq,
...
function initializeHelpCountResult(
	...
	const overallSkillRate = !param.pityProc
		? skillRate
	...
	for (const ing of ret.ingredients) {
		ing.countPerHelp =
			ing.slots.reduce((p, c) => p + c.count, 0) / ing.slots.length;
	}
...
function calculateBaseFreqAndBonus(
	...
	const helpBonusCount =
		param.helpBonusCount + (iv.hasHelpingBonusInActiveSubSkills ? 1 : 0);
...
function calculateTimeToFullInventory(
	...
	const sleepEfficiencies =
		alwaysSnacking || alwaysTapAsleep
			? []
	...
	for (const efficiency of sleepEfficiencies) {
		...
		timeToFullInventory =
			(requiredHelpCount * freq) / 60 +
			efficiency.start -
			sleepEfficiencies[0].start;
		break;
	}
...
function calculateAwakeHelpCount(
	...
	const tapIntervalAwake =
		param.tapFrequencyAwake === AlwaysTap || param.tapFrequencyAwake === NoTap
			? awakeSeconds
			: param.tapFrequencyAwake * 60;
	...
	for (const helpCount of helpCounts) {
		...
		ret.ingredients.forEach((ingredient, index) => {
			...
			ingredient.helpCount +=
				(ingHelpCount / ret.ingSlotCount) * ingredient.slots.length;
	...
	ret.berryHelpCount = ret.berryNormalHelpCount + ret.berrySneakySnackingCount;
}
...
function calculateAsleepHelpCount(
	...
	const tapIntervalAsleep =
		param.tapFrequencyAsleep === AlwaysTap || param.tapFrequencyAsleep === NoTap
			? sleepSeconds
			: param.tapFrequencyAsleep * 60;
	const asleepHelpCounts = calculateHelpCountPerTap(
		energy.efficiencies,
		1440 * 60 - sleepSeconds,
		baseFreq,
		tapIntervalAsleep,
		sleepSeconds,
	);
...
export function calculateNextHelpElapsed(
	...
	const event = efficiencies.find(
		(e) => e.start <= elapsedMin && elapsedMin < e.end,
	);
...
export function calculateHelpCountInInterval(
	...
	while (true) {
		const nextElapsed = calculateNextHelpElapsed(
			efficiencies,
			currentElapsed,
			baseFreq,
		);
		...
		if (nextElapsed >= endTime) {
			...
			const fractionalCount =
				helpDuration === 0 ? 0 : (endTime - currentElapsed) / helpDuration;
...
export class HelpCountSimulation {
	/**
	 * Probability distribution of the used inventory space after the latest
	 * computed step: `state[u]` is the probability that `u` slots are used and
	 * the inventory is not full yet (0 <= u < carryLimit).
	 */
	...
	/**
	 * Expected count of ingredient kind `j` brought by normal helps by step `i`.
	 *
	 * (ex) `cumulativeIngExpected[5][0]` is the expected count
	...
	constructor(
		...
		this.numIngredientKinds = Math.max(
			...this.bagUsage.map((usage) => usage.ingKindIndex + 1),
		);
		...
		this.isSkillSpecialty =
			iv.pokemon.specialty === "Skills" || iv.pokemon.specialty === "All";

		// Initialize state: nothing in the inventory
		this.state = Array.from<number>({ length: this.carryLimit }).fill(0);
		this.state[0] = 1.0;

		// Initialize cummulative arrays
		this.cumulativeFullProb = [0];
		this.cumulativeBerryExpected = [0];
	...
	compute(n: number): HelpCountSimulationResult {
		...
		return {
			...
			overflowIngsPerSlot: lo.overflowIngsPerSlot.map((v, i) =>
				lerp(v, hi.overflowIngsPerSlot[i]),
			),
	...
	private computeN(n: number): HelpCountSimulationResult {
		...
		while (this.cumulativeFullProb.length <= n) {
			this.computeStep();
		}
		...
		const berryCount =
			...
			sneakySnackingCount * this.sneakySnackingBerryCount;
	 ...
	 * used inventory space and accumulating full-inventory probabilities and
	 * the expected item counts added by the helps into the cumulative arrays.
	 */
	private computeStep(): void {
		const stepIndex = this.cumulativeFullProb.length;
		const prevCumulativeOverflow =
			this.cumulativeOverflowIngSlots[stepIndex - 1];

		const nextState = Array.from<number>({ length: this.carryLimit }).fill(0);
		let newFullProb = 0;
		let newBerry = 0;
		...
		for (let used = 0; used < this.carryLimit; used++) {
			const prob = this.state[used];
			if (prob === 0) {
				continue;
			}
			...
			for (const usage of this.bagUsage) {
				...
				if (usage.name === "berry") {
					newBerry += usage.count * transitionProb;
					newUsed += usage.count;
				} else {
					// Ingredient: cap added amount so total doesn't exceed carryLimit
					const space = this.carryLimit - used;
					const addCount = Math.min(usage.count, space);
					const overflow = usage.count - addCount;
					if (overflow > 0) {
						newOverflowIngSlots[usage.ingSlotIndex] +=
							overflow * transitionProb;
					}
					newIng[usage.ingKindIndex] += addCount * transitionProb;
					newUsed += addCount;
				}
		...
		this.cumulativeFullProb.push(
			this.cumulativeFullProb[stepIndex - 1] + newFullProb,
		);
		this.cumulativeBerryExpected.push(
			this.cumulativeBerryExpected[stepIndex - 1] + newBerry,
		);
		this.cumulativeIngExpected.push(
			this.cumulativeIngExpected[stepIndex - 1].map(
				(v: number, i: number) => v + newIng[i],
			),
		);
