...
const totalExpToTheLevel: number[] = [
	0, 0, 54, 125, 233, 361, 525, 727, 971, 1245, 1560, 1905, 2281, 2688, 3107,
	3536, 3976, 4430, 4899, 5382, 5879, 6394, 6931, 7489, 8068, 8668, 9290, 9933,
...
export type GrowthIncensePolicy =
	| "none"
	| "fullMoon"
	| "gsd"
	| "every2Days"
 ...
 */
export default function calcExpAndCandy(
	iv: PokemonIv,
	expGot: number,
	dstLevel: number,
	boost: BoostEvent,
 ...
 */
export function calcLevelByCandy(
	iv: PokemonIv,
	expGot: number,
	dstLevel: number,
	candy: number,
...
function calcExpPerCandy(
	...
	return (
		(nature.isExpGainsUp ? 30 : nature.isExpGainsDown ? 21 : 25) * boostFactor
	);
}
...
export function calcExp(level1: number, level2: number, iv: PokemonIv): number {
	...
	if (level1 < 0 || level1 > maxLevel || level2 < 0 || level2 > maxLevel) {
		return 0;
	}

	return (
		Math.round(totalExpToTheLevel[level2] * rate) -
 ...
 */
export function calcDayToGetSleepExp(
	exp: number,
	expBonus: number,
	score: number,
	expGainRate: number,
	policy: GrowthIncensePolicy,
	today: Date = new Date(),
	...
	while (true) {
		...
		if (dates > 1) {
			...
			const expPerDay = Math.floor(
				baseExp * getExpRateForDay("normal", policy) * expGainRate,
			);
...
function getExpRateForDay(
	date: "normal" | "gsd" | "fullmoon",
	policy: GrowthIncensePolicy,
) {
	const baseExp = date === "fullmoon" ? 3 : date === "gsd" ? 2 : 1;
	switch (policy) {
		case "none":
			return baseExp;
		case "fullMoon":
			return baseExp === 3 ? 6 : baseExp;
		case "gsd":
			return baseExp > 1 ? baseExp * 2 : baseExp;
		case "every2Days":
			return baseExp > 1 ? baseExp * 2 : baseExp * 1.5;
		case "everyDay":
			return baseExp * 2;
	}
}
