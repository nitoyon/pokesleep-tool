...
import { getEnergyByState } from "../TeamEnergy";
import type {
	MemberProfile,
	MemberProgress,
	TeamContext,
...
export function applyHelp(
	...
	for (const member of sim.members) {
		applyHelpForMember(member, sim.teamProfile.param, tapSec, sim, rng);
	}
}
 ...
 *
 * Includes any help count already advanced by an intervening
 * CookEvent/SleepRecoverEvent catch-up (see {@link advanceHelpSchedule}).
...
function calculateHelpCount(
	...
	const ret =
		advanceHelpSchedule(progress, profile, tapSec) + progress.pendingHelp;
	progress.pendingHelp = 0;
	return ret;
}

/**
 * Advance a member's help schedule up to `sec`, without collecting any
 * berries/ingredients (collection stays tap-only).
 *
 * Callers that change energy outside of a tap (CookEvent,
 * SleepRecoverEvent's wake branch) must call this first and fold the
 * result into progress.pendingHelp, so that `lastRecoverySec` never
 * advances past a help completion time that a later tap still needs to
 * replay via {@link scheduleNextHelp}.
 * @param progress Current simulation progress for the member.
 * @param profile Profile for the member.
 * @param sec Target time in seconds.
 * @returns The number of helps completed up to `sec`.
 */
export function advanceHelpSchedule(
	progress: MemberProgress,
	profile: MemberProfile,
	sec: number,
): number {
	// -1 is a sentinel meaning "not yet scheduled"; compute the first help
	// time from energy as of the last known sync point.
	if (progress.nextHelpSec === -1) {
		scheduleNextHelp(progress, profile, progress.lastRecoverySec);
	}

	let ret = 0;
	while (progress.nextHelpSec <= sec) {
...
function applySingleHelp(
	...
	// Sneaky snacking
	if (carryLimitLeft === 0) {
		progress.berryStrength += progress.berry1Strength * profile.iv.berryCount;
	...
	const rand = rng();
	let cumulative = 0;
	let outcome = profile.normalBagUsage[0];
	for (const item of profile.normalBagUsage) {
		cumulative += item.p;
		if (rand < cumulative) {
			outcome = item;
			break;
	...
	if (outcome.name === "berry") {
		progress.berryStrength += progress.berry1Strength * outcome.count;
	...
	if (
		...
		progress.bigBerryStrength += progress.bigBerry1Strength * count;
		carryLimitLeft -= count;
	}
...
function drawSkillCount(
	...
	if (!pityProc) {
		...
		if (profile.maxSkillCount === 1) return 1;
	...
	for (let i = 0; i < helpCount; i++) {
		...
		if (profile.maxSkillCount === 2 && skillCount >= 2) {
			return 2;
		}
		if (profile.maxSkillCount === 1 && skillCount >= 1) {
			return 1;
		}
	}
