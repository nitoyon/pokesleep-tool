/**
 * Shared fixtures for the `src/util/Team` test suites.
 *
 * This file has no `describe`/`test` of its own, so the vitest runner ignores
 * it; the specs import the builders below to construct a minimal team without
 * duplicating ~40 lines of boilerplate each.
 */
