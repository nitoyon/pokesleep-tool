...
describe("PokemonRP", () => {
	describe("frequency", () => {
		...
		test("test with ribbon", () => {
			...
			expect(Math.floor(new PokemonRp(iv.clone({ ribbon: 2 })).frequency)).toBe(
				3827,
			);
			expect(Math.floor(new PokemonRp(iv.clone({ ribbon: 3 })).frequency)).toBe(
				3827,
			);
			expect(Math.floor(new PokemonRp(iv.clone({ ribbon: 4 })).frequency)).toBe(
				3225,
			);
		});
	});
