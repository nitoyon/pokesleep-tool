...
describe("calculateHelpCount", () => {
	test("calculate timeToFullInventory and skillProbabilityAfterWakeup", () => {
		...
		expect(result.asleep.normal).toBeCloseTo(10, 5);
		...
		const result2 = calculateHelpCount(
			iv2,
			param,
			energy2,
			emptyBonusBase,
	...
	const efficiencies = [
		{
			...
			efficiency: 2.222 as const,
			frequencyRate: 0.45 as const,
			isAwake: true,
			isSnacking: false,
			isInPeriod: true,
		},
...
describe("HelpCountSimulation", () => {
		...
		test("Toxel float helps", () => {
			...
			for (let i = 0; i < result26.overflowIngsPerSlot.length; i++) {
				expect(result26.overflowIngsPerSlot[i]).toBeCloseTo(
					(result3.overflowIngsPerSlot[i] - result2.overflowIngsPerSlot[i]) *
						0.6 +
	...
	describe("Probability conservation", () => {
		test("normalHelpCount + sneakySnackingCount equals n for each n", () => {
			...
			for (let n = 1; n <= 15; n++) {
				...
				expect(result.normalHelpCount + result.sneakySnackingCount).toBeCloseTo(
					n,
					10,
				);
			}
	...
	describe("Good camp ticket", () => {
		test("good camp ticket increases carry limit by 20%", () => {
			...
			const totalNormal = resultNormal.ingredientCount.reduce(
				(a, b) => a + b,
				0,
			);
			const totalTicket = resultTicket.ingredientCount.reduce(
				(a, b) => a + b,
				0,
			);
	...
	describe("Monotonicity", () => {
		test("sneakySnackingCount, expectedBerries, and expectedIngredients increase monotonically with N", () => {
			...
			for (let n = 2; n <= 20; n++) {
				...
				expect(curr.sneakySnackingCount).toBeGreaterThanOrEqual(
					prev.sneakySnackingCount,
				);
				expect(curr.berryCount).toBeGreaterThanOrEqual(prev.berryCount);
				for (let j = 0; j < curr.ingredientCount.length; j++) {
					expect(curr.ingredientCount[j]).toBeGreaterThanOrEqual(
						prev.ingredientCount[j],
					);
				}
				prev = curr;
			}
	...
	describe("returns same result as calculateInventoryDistribution", () => {
		test("Venusaur (Lv60 ABC)", () => {
			...
			for (let n = 2; n <= 20; n++) {
				...
				expect(res.normalHelpCount + res.sneakySnackingCount).toBeCloseTo(n);
