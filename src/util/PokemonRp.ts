...
export const averageIngredientStrength: number = (() => {
	const ingredients = Object.entries(ingredientStrength).filter(
		([name]) => !name.startsWith("unknown"),
	);
...
class PokemonRp {
	...
	get frequency(): number {
		return this.getOrCache("frequency", () => {
			...
			const uglyHack =
				this.iv.level === 10 && this.iv.pokemon.frequency === 2600 ? 0.1 : 0;

			return (
				this.iv.pokemon.frequency * // Base frequency
					trunc(
						// Level Factor
						((501 - this.iv.level) / 500) *
							...
							// Sub-Skill Factor
							(1 -
								this.activeSubSkills.reduce((p, c) => p + c.helpingSpeed, 0) *
									0.07),
						4,
					) -
	...
	get ingredientRp(): number {
		return this.getOrCache("ingredientRp", () => {
			return trunc(
				this.helpCountPer5Hour *
					this.iv.ingredientRate *
					this.ingredientEnergy *
					this.ingredientG,
				2,
			);
		});
	...
	get ingredientG(): number {
		return this.getOrCache("ingredientG", () => {
			const table = [
				...
				// Expected values by Claude Code (quadratic fit on Lv61-70)
				// biome-ignore lint/suspicious/noApproximativeNumericConstant: data table value, not Math.E
			...
			// quadratic fit on Lv61-70 confirmed values
			return (
				0.0006515 * this.iv.level ** 2 - 0.032106 * this.iv.level + 1.461109
			);
		});
	}
