...
/**
 * Internal type: Item count distribution from getBagUsagePerHelpDetail.
 */
type ItemDistribution = Array<{count: number, p: number}>;

/**
 * Calculates the probability distribution of when a Pokémon's inventory fills
 * using Dynamic Programming.
 *
 * @param itemDist - Item distribution from getBagUsagePerHelpDetail
 * @param carryLimit - Maximum inventory capacity
 * @returns CDF array where index i = P(filled by help i or earlier)
 */
function calculateInventoryDistributionDP(
    itemDist: ItemDistribution,
    carryLimit: number
): InventoryDistributionResult {
    // Find minimum item count for array length calculation
    const minItemCount = Math.min(...itemDist.map(d => d.count));
    ...
    for (let h = 0; h < maxIterations; h++) {
        ...
        for (const [currentInv, currentProb] of currentState.entries()) {
            ...
            for (const {count, p} of itemDist) {
                const newInv = currentInv + count;

                if (newInv >= carryLimit) {
                    // Inventory fills
                    probFilledThisHelp += currentProb * p;
                } else {
        ...
        cumulativeFilled += probFilledThisHelp;
 ...
 * Calculates the probability distribution of when a Pokémon's inventory fills.
 *
 * Uses Dynamic Programming to provide exact probabilities (no Monte Carlo sampling)
 * with ~10x performance improvement over simulation methods.
 *
 * @param pokemon - PokemonIv instance with carry limit and item rates
...
export function calculateInventoryDistribution(
    ...
    // Get item distribution from PokemonIv
    const itemDist = pokemon.getBagUsagePerHelpDetail(bonus ?? {});

    // Get carry limit
    const carryLimit = pokemon.carryLimit;

    // Calculate and return CDF
    return calculateInventoryDistributionDP(itemDist, carryLimit);
}
