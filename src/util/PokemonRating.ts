...
class PokemonRating {
	...
	calculate(): RatingStrengthResult {
		...
		const berryMaxRp = new PokemonRp(
			this.iv.clone({
				subSkills: new SubSkillList({
					...
					lv70: new SubSkill("Helping Speed S"),
				}),
				nature: new Nature("Adamant"),
			}),
		);

		const ingMaxRp = new PokemonRp(
			this.iv.clone({
				subSkills: new SubSkillList({
					...
					lv50: new SubSkill("Helping Speed M"),
					lv70: new SubSkill("Helping Speed S"),
					lv80: new SubSkill("Helping Bonus"),
				}),
				nature: new Nature("Quiet"),
			}),
		);

		const skillMaxRp = new PokemonRp(
			this.iv.clone({
				subSkills: new SubSkillList({
		...
		const berryCalc = (rp: PokemonRp) => {
			let ret =
				(3600 / rp.iv.frequencyWithHelpingBonus(0)) *
				rp.iv.berryRate *
				rp.iv.berryCount;
		...
		const ingCalc = (rp: PokemonRp) =>
			(3600 / rp.iv.frequencyWithHelpingBonus(0)) * rp.iv.ingredientRate;
		...
		const skillCalc = (rp: PokemonRp) =>
			(3600 / rp.iv.frequencyWithHelpingBonus(0)) * rp.iv.skillRate;
