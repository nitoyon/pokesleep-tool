...
describe("PokemonStrength", () => {
	describe("calculate", () => {
		test("ingredient unlock levels (Lv 1)", () => {
			...
			expect(result.ingHelpCount).toBe(result.ingredients[0].helpCount);
		});
		...
		test("ingredient unlock level (Lv 30)", () => {
			...
			expect(result.ingHelpCount).toBe(
				result.ingredients[0].helpCount + result.ingredients[1].helpCount,
			);
		});
		...
		test("ingredient unlock level (Lv 60 ABB)", () => {
			...
			expect(result.ingredients[0].helpCount).toBeCloseTo(
				result.ingHelpCount / 3,
			);
			expect(result.ingredients[1].helpCount).toBeCloseTo(
				(result.ingHelpCount * 2) / 3,
			);
		});
		...
		test("field bonus affects berry and ingredient strength", () => {
			...
			expect(result75.berryTotalStrength).toBeCloseTo(
				result0.berryTotalStrength * 1.75,
			);
		...
		test("nature affects results", () => {
			...
			expect(resultBold.berryHelpCount + resultBold.ingHelpCount).toBeLessThan(
				resultSerious.berryHelpCount + resultSerious.ingHelpCount,
			);
		});
		...
		test("ingredients summary", () => {
			...
			const sumStrength = result.ingredients.reduce(
				(sum, ing) => sum + ing.strength,
				0,
			);
	...
	describe("expert mode", () => {
		...
		describe("ExpertEffects", () => {
			test("berry: favorite has higher berryStrengthBonus than non-favorite", () => {
				const iv = new PokemonIv({ pokemonName: "Raichu", level: 50 });
				const strengthFav = new PokemonStrength(
					iv,
					createParam({
						fieldIndex,
						favoriteType: ["electric", "fire", "water"],
				...
				const strengthNonFav = new PokemonStrength(
					iv,
					createParam({
						fieldIndex,
						favoriteType: ["fire", "water", "grass"],
				...
				expect(strengthFav.berryStrengthBonus).toBeGreaterThan(
					strengthNonFav.berryStrengthBonus,
				);
			});
			...
			test("skill: expertEffect skill gives higher skillCount than expertEffect berry", () => {
				...
				const resultBerry = new PokemonStrength(
					iv,
					createParam({
						fieldIndex,
						favoriteType: ["electric", "fire", "water"],
				...
				expect(resultSkill.skillCount).toBeCloseTo(
					resultBerry.skillCount * 1.25,
				);
			});
		});
		...
		describe("speed effects", () => {
			test("main berry has more total helps than sub berry (10% speed bonus)", () => {
				const iv = new PokemonIv({ pokemonName: "Raichu", level: 50 });
				const resultMain = new PokemonStrength(
					iv,
					createParam({
						fieldIndex,
						favoriteType: ["electric", "fire", "water"],
				...
				const resultSub = new PokemonStrength(
					iv,
					createParam({
						fieldIndex,
						favoriteType: ["fire", "electric", "water"],
			...
			test("non-favorite has fewer total helps than sub berry (15% speed penalty)", () => {
				...
				const resultNonFav = new PokemonStrength(
					iv,
					createParam({
						fieldIndex: 7,
						favoriteType: ["fire", "water", "grass"],
		...
		describe("main berry skill level bonus", () => {
			test("main berry pokemon has higher skillValuePerTrigger than sub berry (Slaking, Ingredient Magnet S)", () => {
				...
				expect(resultMain.skillValuePerTrigger).toBeGreaterThan(
					resultSub.skillValuePerTrigger,
				);
			});
		});
	});
	...
	describe("energy.helpCount (asleepNotFull / asleepFull)", () => {
		test("period=8 (shorter than sleep start at 15.5h): sleep counts are 0", () => {
			const iv = new PokemonIv({ pokemonName: "Raichu", level: 50 });
			const result = new PokemonStrength(
				iv,
				createParam({ period: 8 }),
			).calculate();
		...
		test("longer period covers more sleep time: period=24 has more sleep helps than period=16", () => {
			...
			const result16 = new PokemonStrength(
				iv,
				createParam({
					period: 16,
					tapFrequencyAsleep: AlwaysTap,
			...
			const result24 = new PokemonStrength(
				iv,
				createParam({
					period: 24,
					tapFrequencyAsleep: AlwaysTap,
			...
			expect(result24.asleep.normal).toBeGreaterThan(result16.asleep.normal);
		});
		...
		test("period=168 (1 week): energy.helpCount equals period=24 (single daily cycle)", () => {
			...
			const result168 = new PokemonStrength(
				iv,
				createParam({
					period: 168,
					tapFrequencyAsleep: AlwaysTap,
			...
			expect(result168.asleep.normal).toBeCloseTo(result24.asleep.normal * 7);
			...
			expect(result168.berryTotalStrength).toBeCloseTo(
				result24.berryTotalStrength * 7,
			);
		...
		test("inventory filling during sleep: NoTap splits into asleepNotFull and asleepFull", () => {
			...
			const resultNoTap = new PokemonStrength(
				iv,
				createParam({
					period: 24,
					tapFrequencyAwake: AlwaysTap,
			...
			const resultAlwaysTap = new PokemonStrength(
				iv,
				createParam({
					period: 24,
					tapFrequencyAwake: AlwaysTap,
	...
	describe("helpCount", () => {
		test("period=-10 (10 fixed helps)", () => {
			...
			expect(result.total.sneakySnacking).toBe(0);
			expect(result.total.all).toBeCloseTo(
				result.berryHelpCount + result.ingHelpCount,
				4,
			);
			...
			expect(result.ingredients[0].count).toBe(
				(result.ingHelpCount * result.ing1.count) / 3,
			);
			expect(result.ing2.count).toBe(2);
			expect(result.ingredients[1].count).toBe(
				(result.ingHelpCount * result.ing2.count) / 3,
			);
		...
		test("AlwaysTap (8 hours)", () => {
			...
			expect(result.berryHelpCount).toBe(result.berryNormalHelpCount);
			expect(result.berrySneakySnackingCount).toBe(0);
			expect(result.berryCount).toBe(
				result.berryHelpCount * result.berryCountPerNormalHelp,
			);
			expect(result.ing1.count).toBe(1);
			expect(result.ingredients[0].count).toBe(
				(result.ingHelpCount * result.ing1.count) / 3,
	...
	describe("Mew base rate overrides", () => {
		test("Mew uses param.mew", () => {
			...
			// skill3
			const strength3 = new PokemonStrength(
				new PokemonIv({
					pokemonName: "Mew",
