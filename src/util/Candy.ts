 ...
 */
export default function calcCandyUsage(
	required: number,
	bag: CandyCount,
): CandyCount[] {
	...
	function dfs(
		...
		const minCount =
			stepIndex === steps.length - 1
				? Math.ceil(remaining / candyType.value)
				: 0;
	...
	return results
		.slice(0, 10)
		.map(({ handyS, handyM, handyL, typeS, typeM, typeL }) => ({
			handyS,
			handyM,
			handyL,
			typeS,
...
export function calcMaxCandy(bag: CandyCount): number {
	return candyTypes.reduce(
		(sum, candy) => sum + bag[candy.key] * candy.value,
		0,
	);
}
