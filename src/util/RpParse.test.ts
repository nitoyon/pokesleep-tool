...
describe("Energy", () => {
	...
	test("Lvl25-29", () => {
		const ret = parseTsv(
			"Abomasnow	25	1053	Quiet	1	Ingredient Finder S	Inventory Up M	",
		);
		expect("Abomasnow" in ret).toBe(true);
		expect(ret["Abomasnow"].length).toBe(1);
		...
		expect(ret["Abomasnow"][0].iv.skillLevel).toBe(1);
		expect(ret["Abomasnow"][0].iv.subSkills.lv10?.name).toBe(
			"Ingredient Finder S",
		);
	...
	test("Lvl30-49", () => {
		const ret = parseTsv(
			"Abomasnow	30	1580	Bashful	2	Helping Speed M	Ingredient Finder M	Fancy Egg",
		);
		expect("Abomasnow" in ret).toBe(true);
		expect(ret["Abomasnow"].length).toBe(1);
		...
		expect(ret["Abomasnow"][0].iv.subSkills.lv25?.name).toBe(
			"Ingredient Finder M",
		);
	...
	test("Lvl50-59", () => {
		const ret = parseTsv(
			"Blastoise	50	3025	Impish	3	Berry Finding S	Ingredient Finder M	Helping Speed M	Soothing Cacao",
		);
		...
		expect(ret["Blastoise"][0].iv.subSkills.lv25?.name).toBe(
			"Ingredient Finder M",
		);
	...
	test("Lvl60-", () => {
		const ret = parseTsv(
			"Blastoise	60	3692	Lonely	3	Ingredient Finder M	Ingredient Finder S	Skill Trigger M	Soothing Cacao	Soothing Cacao",
		);
		expect("Blastoise" in ret).toBe(true);
		expect(ret["Blastoise"].length).toBe(1);
		...
		expect(ret["Blastoise"][0].iv.skillLevel).toBe(3);
		expect(ret["Blastoise"][0].iv.subSkills.lv10?.name).toBe(
			"Ingredient Finder M",
		);
		expect(ret["Blastoise"][0].iv.subSkills.lv25?.name).toBe(
			"Ingredient Finder S",
	...
	test("Submissions 30", () => {
		const ret = parseTsv(
			"Comfey	30	1380	Gentle	1	Skill Trigger S	Ingredient Finder S		Warming Ginger	",
		);
		...
		expect(ret["Comfey"][0].iv.subSkills.lv25?.name).toBe(
			"Ingredient Finder S",
		);
