...
describe("Energy", () => {
	...
	test("calculate (e4e x 0, period 8, score: 0)", () => {
		...
		const result = energy.calculate(
			createParam({ e4eCount: 0, period: 8, sleepScore: 0 }),
		);
	...
	test("calculate (e4e x 0, Energy recovery down)", () => {
		...
		if (emptyEvent === undefined || sleepEvent === undefined) {
			throw new Error();
		}
		expect(
			emptyEvent.minutes === sleepEvent.minutes + sleepEvent.energyAfter * 10,
		);
	});
	...
	test("calculate (e4e x 0, Energy Recovery Bonus)", () => {
		const iv = new PokemonIv({
			...
			subSkills: new SubSkillList({
				lv10: new SubSkill("Energy Recovery Bonus"),
			}),
		});
		const energy = new Energy(iv);
		const result = energy.calculate(createParam({ e4eCount: 0 }));
	...
	test("calculate (e4e x 0, Energy recovery down, Energy Recovery Bonus)", () => {
		...
		result = energy.calculate(
			createParam({ e4eCount: 0, recoveryBonusCount: 1 }),
		);
	...
	test("calculate (score 0)", () => {
		...
		const energy = new Energy(iv);
		const result = energy.calculate(
			createParam({ e4eCount: 0, sleepScore: 0 }),
		);
