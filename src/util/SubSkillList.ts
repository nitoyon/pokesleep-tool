...
class SubSkillList {
	...
	clone(updates?: Partial<SubSkillListProps>): SubSkillList {
		...
		const applyUpdate = (
			level: keyof SubSkillListProps,
			subSkill: SubSkill | null,
		) => {
			if (subSkill !== null) {
				// Find if this skill already exists at another level
				const existingLevel = (
					Object.keys(newValue) as (keyof SubSkillListProps)[]
		...
		if (updates.lv10 !== undefined) {
			applyUpdate("lv10", updates.lv10);
		}
	...
	getActiveSubSkills(level: number): SubSkill[] {
		...
		if (this.value.lv10 !== null) {
			ret.push(this.value.lv10);
		}

		if (level < 25) {
			return ret;
		}
		if (this.value.lv25 !== null) {
			ret.push(this.value.lv25);
		}

		if (level < 50) {
			return ret;
		}
		if (this.value.lv50 !== null) {
	...
	getSubSkillIndex(subSkill: SubSkill | null) {
		...
		if (this.value.lv10?.name === subSkill.name) {
			return 0;
		}
		if (this.value.lv25?.name === subSkill.name) {
			return 1;
		}
	...
	get lv10(): SubSkill | null {
		return this.value.lv10;
	}
	/** Get the sub skill for level 25. */
	get lv25(): SubSkill | null {
		return this.value.lv25;
	}
