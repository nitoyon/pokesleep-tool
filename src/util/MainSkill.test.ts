	...
	describe("matchMainSkillName", () => {
		...
		describe("Mew versatile special cases", () => {
			test("Mew with versatileSkill set matches that skill", () => {
				...
				expect(matchMainSkillName(mew, "Charge Strength M", false, iv)).toBe(
					true,
				);
				expect(matchMainSkillName(mew, "Ingredient Magnet S", false, iv)).toBe(
					false,
				);
			});
		...
		describe("Toxel evolution special cases", () => {
			test("Toxel with Amped nature evolves to Ingredient Magnet S (Plus)", () => {
				...
				expect(matchMainSkillName(toxel, "Ingredient Magnet S", true, iv)).toBe(
					true,
				);
				expect(matchMainSkillName(toxel, "Cooking Power-Up S", true, iv)).toBe(
					false,
				);
			});
			...
			test("Non-Toxel pokemon ignores evolution parameters", () => {
				...
				expect(matchMainSkillName(pikachu, "Charge Strength S", true, iv)).toBe(
					true,
				);
				expect(
