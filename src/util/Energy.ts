...
class Energy {
    ...
    getEfficiencyChangeMinutes(curEvent: EnergyEvent, nextEvent: EnergyEvent
    ): number|null {
        const curEnergy = curEvent.energyAfter;

        // get the minute when the efficiency changes
        // ex) If curEnergy is 94, thresholdEnergy is 80 and
        //     thresholdMinute is 140 minutes later.
        const thresholdEnergy = curEnergy % 20 === 0 ? curEnergy - 20 :
            Math.floor(curEnergy / 20) * 20;
        const thresholdMinutes = curEvent.minutes +
            (curEnergy - thresholdEnergy) * 10;

        if (thresholdMinutes < nextEvent.minutes) {
            return thresholdMinutes;
        }

        // returns null when efficiency doesn't change
        return null;
    }
