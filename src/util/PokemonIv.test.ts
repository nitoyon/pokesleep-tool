...
describe("PokemonIV", () => {
	...
	describe("pity proc", () => {
		test("Darkrai", () => {
			...
			expect(iv.calculateSkillRateWithPityProc(iv.skillRate)).toBeCloseTo(
				0.034,
			);
		});
	});
	...
	describe("normalize (static)", () => {
		test("throws error when pokemonName is missing", () => {
			expect(() => PokemonIv.normalize({})).toThrow("pokemonName is required");
		});

		test("throws error when pokemonName is invalid", () => {
			expect(() =>
				PokemonIv.normalize({ pokemonName: "InvalidPokemon" }),
	...
	describe("getBagUsagePerHelp", () => {
		...
		test("level 60+ uses average of all three ingredients", () => {
			...
			const result = iv.getBagUsagePerHelp();
			const avgIngCount =
				(iv.ingredient1.count + iv.ingredient2.count + iv.ingredient3.count) /
				3;
		...
		test("with expert bonus for Ingredients specialty Pokemon (level = 30)", () => {
			...
			const expectedBerryPart = iv.berryRate * 1;
			const expectedIngPart =
				(iv.ingredientRate *
					(iv.ingredient1.count + 1.5 + iv.ingredient2.count + 1.5)) /
				2;
			expect(result).toBeCloseTo(expectedBerryPart + expectedIngPart);
		});
		...
		test("level 60+ with ingredient bonus", () => {
			...
			const avgIngCount =
				(iv.ingredient1.count +
					iv.ingredient2.count +
					iv.ingredient3.count +
					3) /
				3;
			const expectedBerryPart = iv.berryRate * 1;
			const expectedIngPart = iv.ingredientRate * avgIngCount;
